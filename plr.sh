...
cfg.read(os.path.expanduser('~/printer_data/config/variables.cfg'))
key = sys.argv[1]
...
thumb_end=$(grep -n '; thumbnail end' $HOME/plrtmpA.$$ | tail -1 | cut -d: -f1)
if [ -n "${thumb_end}" ]; then
  head -n "${thumb_end}" $HOME/plrtmpA.$$ > ${PLR_PATH}/"${plr}"
else
  : > ${PLR_PATH}/"${plr}"
fi
